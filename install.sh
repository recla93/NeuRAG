...
PY
    ...
    GMWHEEL=$(ls "$HERE"/_gm_vendor/gray_matter-*.whl 2>/dev/null | head -n 1)
    [ -n "$GMWHEEL" ] && "$VPY" -m pip install --no-deps --no-index "$GMWHEEL" >/dev/null 2>&1 || true
    # Coming FROM a gateway install? Release NeuRAG BEFORE the direct
    ...
    if [ -f "$SUITE/graymatter/manifest.json" ] || [ -f "$SUITE/graymatter/settings.json" ]; then
        echo "Gray Matter detected: releasing NeuRAG from gateway management..."
        "$VPY" -c "from gray_matter.clients import release_tool; [print('  ' + l) for l in release_tool('neurag')]" 2>/dev/null \
            || echo "  (!) automatic release failed — complete the switch with: neurag go-standalone"
    fi
